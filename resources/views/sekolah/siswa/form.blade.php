<section id="content" style="overflow: visible;">
    ...
    <div class="container">
      <h1>{{ empty($siswa) ? "Tambah Siswa" : "Edit Siswa" }}</h1>
      <hr color="black" style="height: 2px;">
    </div>

    <div class="container">
      <form method="POST" id="form_sekolah" action="{{ empty($siswa) ? url(auth()->user()->level.'/siswa') : url(auth()->user()->level.'/siswa/'.$siswa->id) }}">
       @csrf
       @if( !empty($siswa) )
       ...
       <div class="form-row">
        <div class="form-group col-md-6">
         ...
         <input type="text" name="nik" @if(!empty($siswa)) value="{{ $siswa->nik }}" @endif class="form-control" id="nik" placeholder="">
       </div>
       <div class="form-group col-md-6">
         <label for="nama_lengkap">Nama Lengkap</label>
         <input type="text" name="nama_lengkap" @if(!empty($siswa)) value="{{ $siswa->nama_lengkap }}" @endif class="form-control" id="nama_lengkap" placeholder="">
       </div>
     </div>
     ...
     <div class="form-row">
       <div class="form-group col-md-6">
         ...
         <div class="form-check form-check-inline">
          <input class="form-check-input" type="radio" name="jenis_kelamin" @if(!empty($siswa)) @if ($siswa->jenis_kelamin == 'Laki-laki') checked @endif @endif id="Laki-laki" value="Laki-laki">
          <label class="form-check-label" for="Laki-laki">{{ __('Laki-laki') }}</label>
        </div>

        <div class="form-check form-check-inline">
          <input class="form-check-input" type="radio" name="jenis_kelamin" @if(!empty($siswa))  @if ($siswa->jenis_kelamin == 'Perempuan') checked @endif @endif id="Perempuan" value="Perempuan">
      ...
      <div class="form-group col-md-6">
       ...
       <input type="date" name="tanggal_lahir" @if(!empty($siswa)) value="{{ $siswa->tanggal_lahir }}" @endif class="form-control" id="tanggal_lahir" placeholder="">
     </div>
   </div>
   ...
   <div class="form-row">
     <div class="form-group col-md-6">
       ...
       <textarea name="alamat" id="alamat" class="form-control" cols="20" rows="10">@if(!empty($siswa)) {{ $siswa->alamat }} @endif</textarea>
     </div>
     <input type="hidden" name="sekolah_id" @if(!empty($siswa)) value="{{ $siswa->sekolah_id }}" @else value="{{ $sekolah_id->id }}" @endif >
     {{-- <div class="form-group col-md-6">
...
<div class="form-row">
  ...
  <div class="form-group col-md-6">
   ...
   <a href="{{ url('sekolah/siswa') }}" class="btn btn-danger float-right mr-2">Kembali</a>
 </div>
</div>
